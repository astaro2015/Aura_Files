...
/**
 * DjVu ZP (Z-Prime) adaptive binary arithmetic decoder.
 *
 * Pure Java translation/adaptation of the MIT-licensed clean implementation
 * in matyushkin/djvu-rs, which in turn is written from the public DjVu v3
 * specification. No DjVuLibre/GPL code is used here.
 */
final class ZpDecoder {
    ...
    // Only states 0..82 have non-zero adaptation thresholds.
    private static final int[] THRESHOLD_PREFIX = {
    ...
    private static final int[] MPS_NEXT = {
    ...
    private static final int[] LPS_NEXT = {
    ...
    ZpDecoder(byte[] data) throws DjvuException {
        this(data, 0, data == null ? 0 : data.length);
    }

    ZpDecoder(byte[] data, int offset, int length) throws DjvuException {
        if (data == null || length < 2 || offset < 0 || offset + length > data.length) {
            throw new DjvuException("ZP: поток слишком короткий");
        }
        ...
        c = ((high << 8) | low) & 0xffff;
        refillBuffer();
        fence = Math.min(c, 0x7fff);
    }

    boolean decode(int[] ctx, int index) {
        int state = ctx[index] & 0xff;
        int mps = state & 1;
        long z = (long) a + PROB[state];
        if (z <= fence) {
            a = (int) z & 0xffff;
            return mps != 0;
        }
        long boundary = 0x6000L + (((long) a + z) >>> 2);
        int zc = (int) Math.min(z, boundary);
        if (zc > c) {
            int lps = 1 - mps;
            int complement = 0x10000 - zc;
            a = (a + complement) & 0xffff;
            c = (c + complement) & 0xffff;
            ctx[index] = LPS_NEXT[state];
            renormalize();
            return lps != 0;
        } else {
            int threshold = state < THRESHOLD_PREFIX.length ? THRESHOLD_PREFIX[state] : 0;
            if (a >= threshold) ctx[index] = MPS_NEXT[state];
            bitCount--;
            a = (zc << 1) & 0xffff;
            c = ((c << 1) | ((bitBuf >>> bitCount) & 1)) & 0xffff;
            if (bitCount < 16) refillBuffer();
            fence = Math.min(c, 0x7fff);
            return mps != 0;
        }
    }

    boolean decode(int[] oneCtx) { return decode(oneCtx, 0); }

    boolean decodePassthrough() {
        int z = (0x8000 + (a >>> 1)) & 0xffff;
        return passthrough(z);
    }

    boolean decodeIw() {
        int z = (int) (0x8000L + (3L * a) / 8L) & 0xffff;
        return passthrough(z);
    }

    boolean isExhausted() { return pos >= data.length; }

    private boolean passthrough(int z) {
        if (z > c) {
            int complement = 0x10000 - z;
            a = (a + complement) & 0xffff;
            c = (c + complement) & 0xffff;
            renormalize();
            return true;
    ...
    private void refillBuffer() {
        while (bitCount <= 24) {
            bitBuf = (bitBuf << 8) | readByte();
            bitCount += 8;
        }
    }

    private void renormalize() {
        int shift = countLeadingOnes16(a);
        bitCount -= shift;
        a = (a << shift) & 0xffff;
    ...
    private static int countLeadingOnes16(int x) {
        x &= 0xffff;
        int n = 0;
        for (int bit = 0x8000; bit != 0 && (x & bit) != 0; bit >>>= 1) n++;
        return n;
    }
}
